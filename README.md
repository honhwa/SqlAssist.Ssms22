...
| `stacked` | 擺在游標所在行的上下方，寬度吃滿編輯器、高度收到 45% |

`beside` 的落點完全交給平台，離清單多遠也是它算的——**建議清單為自己的說明
提示保留的空間也算在內**，所以視窗有時會被推得比看起來該有的位置更靠外側。
這段距離沒有公開 API 可以查詢或修正。一百多個欄位的資料表在 620 寬裡永遠要
橫向捲，那就改用 `stacked`：位置不再受清單寬度影響，欄位表也能一次攤開好幾欄，
代價是會蓋住上下幾行程式碼。

`stacked` 的寬度由編輯器決定，因此拖曳握把只會寫回高度，不會蓋掉為 `beside`
調好的那個寬度。

#### 圓角

四個角是請 **DWM** 裁的，不是 WPF 畫的。空間保留機制的 `Popup` 是平台自己
建立的，拿不到它就無法在開啟前設 `AllowsTransparency`；沒有透明度時
`CornerRadius` 只會在角落留下承載視窗的方形底色。改用
`DWMWA_WINDOW_CORNER_PREFERENCE` 讓作業系統裁切整個視窗區域，方形底色與畫進
角落的內容一起被裁掉，成本落在合成器上。

這個屬性只有 Windows 11 有，而且**不先判斷版本**——沒有相容性資訊清單的行程裡
`Environment.OSVersion` 會謊報成 Windows 8，直接看 DWM 回傳的 HRESULT 比猜版本
可靠。裁切沒生效時邊框也維持直角，不會先變圓而露出黑色的三角形。
