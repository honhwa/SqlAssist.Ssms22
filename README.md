...
一行放得下就放一行，那是絕大多數情形。超過 120 個字元才換行，並對齊原本 `*`
的位置——一張一百多欄的資料表攤成一行，等於逼使用者橫向捲動去讀自己剛剛
產生的東西。第一個欄位永遠留在原地，把它推到下一行只會讓 `SELECT` 孤零零地
留在上一行。前導空白裡的定位字元原樣保留、其餘換成空白：定位字元換成空白會讓
對齊在定位寬度不是 4 的機器上跑掉。

換行寬度不做成設定。使用者感覺得到的只有「一行還是好幾行」，分界點在哪裡
他不會有意見。
