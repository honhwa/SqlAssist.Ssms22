...
標題會帶上欄位總數，欄位過多時只顯示前 40 個。提示視窗與建議清單的說明面板
都不能捲動也不能選取複製，所以再怎麼放寬都會有看不完的資料表——
**`Ctrl+Alt+Shift+C` 把游標處物件的完整結構複製到剪貼簿**，貼到哪裡看都可以。
同一個命令也在「工具 → SqlAssist → 複製游標處物件的結構」。
...
複製游標處物件的結構 / 顯示診斷狀態 / 重新整理建議 / 設定… / 編輯 settings.json
非同步建議追蹤
```

快捷鍵：`Ctrl+Alt+Shift+S` 全域開關、`Ctrl+Alt+Shift+C` 複製物件結構
