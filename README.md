...
src/SqlAssist.Ssms22     net48 VSIX
  ...
  Structure/             可停駐的物件結構面板
  Options/               工具→選項 的設定頁
```

核心邏輯刻意集中在沒有 Visual Studio 相依的兩個專案，因此排名、解析與
中繼資料對應都可以在不啟動 SSMS 的情況下驗證。目前共 328 項單元測試。
...
所以最後一行是可點擊的**「開啟完整結構」**，看不完的那一半交給結構面板。
...
點提示裡的「開啟完整結構」，或「工具 → SqlAssist → 開啟游標處物件的結構面板」，
會開啟可停駐的 **SqlAssist 物件結構** 工具視窗：
...
| 指令碼 | 可直接執行的完整 CREATE 指令碼 |

空的分頁不會出現。狀態列直接寫出摘要，例如
`Table  23 個欄位　PK：Id ASC　3 個索引　1 個外來鍵`。

**面板開著時會跟著建議清單的選取走。** `SELECT * FROM C` 列出一票 C 開頭的物件時，
方向鍵移到哪一項就顯示哪一項的完整結構，用來一個一個確認是不是要的那張表。
面板沒開就什麼都不會發生，也不會搶走鍵盤焦點。

跟隨時不會掃過幾個物件就送出幾次查詢：快取裡有就立刻畫出來，沒有的話先顯示
標題與「載入中…」，等選取停下來 200 毫秒才真的去查。

資料格以儲存格為選取單位，可以用滑鼠拉選再 `Ctrl+C`（含標題），
右鍵也有「複製選取內容」與「全選並複製」。「複製指令碼」按鈕會複製整段
CREATE 指令碼：主索引鍵寫進 CREATE TABLE 的條件約束，唯一條件約束寫成
ALTER TABLE，計算欄位寫成 `AS 運算式`——貼上去就能執行。

「重新整理」只丟掉當前物件的快取，不會讓整個資料庫的物件清單跟著重來一次。
...
開啟游標處物件的結構面板 / 顯示診斷狀態 / 重新整理建議 / 設定… / 編輯 settings.json
非同步建議追蹤
```
