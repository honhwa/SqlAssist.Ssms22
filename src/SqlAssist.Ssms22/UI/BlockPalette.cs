...
internal static class BlockPalette
{
        ...
        Color accent, string? preference, bool highContrast, SqlAssistSettings? settings = null)
    {
        ...
        var symbol = Endpoint(ReadOptionalColor(settings?.BlockSymbolForeground) ?? globalInk, settings?.BlockSymbolBackground, inherited ?? SymbolMark, adjustFallback: inherited is not null);

        (Color Foreground, Color Background) Endpoint(Color? explicitInk, string? backgroundPreference, Color? defaultBackground = null, bool adjustFallback = true)
