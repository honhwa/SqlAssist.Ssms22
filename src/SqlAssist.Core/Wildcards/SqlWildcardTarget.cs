...
public sealed class SqlWildcardTarget
{
    ...
    /// </summary>
    /// <remarks>
    /// 三種情形要加：使用者自己寫了限定字（<c>a.*</c>）、敘述裡有兩個以上的資料來源
    /// ——那時不加限定字的欄位名稱可能模稜兩可，展開出來的 SQL 直接執行不了——
    /// 或唯一的那個來源自己寫了別名（<c>FROM dbo.Loan l</c>）。
    ///
    /// 最後那一種不加也執行得動，加的理由是使用者已經用別名說話了：接著再
    /// <c>JOIN</c> 一張表進來時，兩百個欄位得靠他自己一個一個補前綴。
    /// 沒有別名的單一來源不加，那只是多一批他沒有要求的字。
    /// </remarks>
