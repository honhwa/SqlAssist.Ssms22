...
public static class SqlFunctionCatalog
{
    ...
    private static readonly (string Name, string Signature)[] Definitions =
    {
        ...
        ("PERCENTILE_CONT", "PERCENTILE_CONT(percentile) WITHIN GROUP (ORDER BY expression) OVER (...)"),
        ("PERCENTILE_DISC", "PERCENTILE_DISC(percentile) WITHIN GROUP (ORDER BY expression) OVER (...)"),
        ...
        ("FORMATMESSAGE", "FORMATMESSAGE(message | messageNumber [, parameter ...])"),
        ("LEFT", "LEFT(value, length)"),
        ("LEN", "LEN(value)"),
        ("LOWER", "LOWER(value)"),
        ("LTRIM", "LTRIM(value)"),
        ("NCHAR", "NCHAR(code)"),
        ("PARSENAME", "PARSENAME(name, part)"),
        ...
        ("STRING_ESCAPE", "STRING_ESCAPE(value, type)"),
        ...
        ("DATETIMEOFFSETFROMPARTS", "DATETIMEOFFSETFROMPARTS(year, month, day, hour, minute, seconds, fractions, hourOffset, minuteOffset, precision)"),
        ...
        ("COL_LENGTH", "COL_LENGTH(tableName, columnName)"),
        ("COL_NAME", "COL_NAME(tableId, columnId)"),
        ("COLUMNPROPERTY", "COLUMNPROPERTY(objectId, columnName, property)"),
        ("CONNECTIONPROPERTY", "CONNECTIONPROPERTY(property)"),
        ("CONTEXT_INFO", "CONTEXT_INFO()"),
        ("DATABASEPROPERTYEX", "DATABASEPROPERTYEX(databaseName, property)"),
        ...
        ("INDEX_COL", "INDEX_COL(tableName, indexId, keyId)"),
        ("INDEXPROPERTY", "INDEXPROPERTY(objectId, indexName, property)"),
        ("NEWID", "NEWID()"),
        ("NEWSEQUENTIALID", "NEWSEQUENTIALID()"),
        ("OBJECT_DEFINITION", "OBJECT_DEFINITION(objectId)"),
        ("OBJECT_ID", "OBJECT_ID(name [, type])"),
        ("OBJECT_NAME", "OBJECT_NAME(objectId)"),
        ("OBJECTPROPERTY", "OBJECTPROPERTY(objectId, property)"),
        ("OBJECTPROPERTYEX", "OBJECTPROPERTYEX(objectId, property)"),
        ("OBJECT_SCHEMA_NAME", "OBJECT_SCHEMA_NAME(objectId [, databaseId])"),
        ...
        ("STATS_DATE", "STATS_DATE(objectId, statsId)"),
        ("SUSER_SNAME", "SUSER_SNAME([sid])"),
        ("TYPE_ID", "TYPE_ID(typeName)"),
        ("TYPE_NAME", "TYPE_NAME(typeId)"),
        ("TYPEPROPERTY", "TYPEPROPERTY(typeName, property)"),
        ("USER_NAME", "USER_NAME([userId])"),

        // 安全性與權限
        ("DATABASE_PRINCIPAL_ID", "DATABASE_PRINCIPAL_ID([principalName])"),
        ("HAS_PERMS_BY_NAME", "HAS_PERMS_BY_NAME(securable, securableClass, permission)"),
        ("IS_MEMBER", "IS_MEMBER(groupOrRole)"),
        ("IS_OBJECTSIGNED", "IS_OBJECTSIGNED(class, objectId, type, thumbprint)"),
        ("IS_ROLEMEMBER", "IS_ROLEMEMBER(role [, principal])"),
        ("IS_SRVROLEMEMBER", "IS_SRVROLEMEMBER(role [, login])"),
        ("ORIGINAL_LOGIN", "ORIGINAL_LOGIN()"),
        ("SUSER_ID", "SUSER_ID([login])"),
        ("SUSER_NAME", "SUSER_NAME([serverUserId])"),
        ("SUSER_SID", "SUSER_SID([login])"),
        ("USER_ID", "USER_ID([userName])"),

        // 加密與壓縮
        ("COMPRESS", "COMPRESS(expression)"),
        ("CRYPT_GEN_RANDOM", "CRYPT_GEN_RANDOM(length [, seed])"),
        ("DECOMPRESS", "DECOMPRESS(expression)"),
        ("DECRYPTBYPASSPHRASE", "DECRYPTBYPASSPHRASE(passphrase, cipherText)"),
        ("ENCRYPTBYPASSPHRASE", "ENCRYPTBYPASSPHRASE(passphrase, clearText)"),
        ("HASHBYTES", "HASHBYTES(algorithm, expression)"),
