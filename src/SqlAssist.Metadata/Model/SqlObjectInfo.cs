...
public sealed class SqlObjectInfo
{
    ...
    /// 這個物件所屬的資料庫。從中繼資料來的物件一律帶著它——同一條連線的物件帶的是
    /// 查詢視窗選取的那一個，指名別庫時帶的是被指名的那一個；指令碼自己宣告的暫存
    /// 資料表與資料表變數沒有資料庫可言，那時傳 null。連線還沒有選定資料庫時是空字串。
    /// </param>
    ...
    /// <summary>
    /// 這個物件所屬的資料庫；沒有資料庫可言時為 null（只可能是指令碼自己宣告的
    /// 暫存資料表與資料表變數），連線還沒有選定資料庫時是空字串。
    /// </summary>
    /// <remarks>
    ...
    ///
    /// 空字串與 null 都不能拿來組 <c>USE</c>：連線還沒選定資料庫時，那個名稱會是
    /// 兩個方括號而已。要寫進指令碼的地方一律先問「有沒有值」。
    /// </remarks>
