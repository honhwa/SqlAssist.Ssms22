...
/// 啟動時查不到而畫面上只說「找不到」。
///
/// <see cref="ServerName"/> 必填，理由與 <see cref="SqlCatalogSearchTarget.DatabaseName"/>
/// 完全相同：作業<b>只在它自己那台伺服器上</b>存在，而一份結果清單本來就跨伺服器
/// （工具窗的伺服器下拉會換）。少了它，下游會拿現在這條連線去開另一台的作業，
/// 而開出來的東西看起來完全正常。
///
/// 不可變：同一筆結果會同時被清單、預覽與啟動讀到。
/// </remarks>
public sealed class SqlAgentJobSearchTarget
{
