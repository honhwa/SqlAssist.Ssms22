...
模組的指令碼最前面多一個批次，把整份釘在**物件所在**的那個資料庫上：
...
新開的查詢視窗沿用目前連線，而那一條連線不一定連在物件所在的資料庫上——三段式
名稱指到的物件與 SQL Search 指名的那一筆都是。少了這一句，展開出來的
`ALTER PROCEDURE` 會在錯的資料庫上執行；那一個資料庫上剛好有同名的程序時，
它會靜靜地被蓋掉，畫面上看不出差別。

它自成一個批次，理由與那三個 `GO` 相同：模組必須是批次裡的第一個敘述。
三個表面都有這一句——預覽不執行，但少了一邊的症狀是使用者看著預覽對照，
卻在開出來的視窗裡找不到它。

**只對模組寫。**模組的定義是存在資料庫裡的一份文件，`ALTER` 它就是在改那一份；
一份 `CREATE TABLE` 則是可以拿到任何地方執行的樣板，釘在來源資料庫上反而擋掉了
「把這張表編寫到另一個資料庫」這個正常用法。查不到資料庫名稱時整句不寫。
