...
三種情形加：有兩個以上的資料來源、使用者自己寫了限定字、或唯一的那個來源自己寫了別名。
第一種不加會壞掉——`SELECT Name FROM A a JOIN B b` 的欄位名稱模稜兩可，執行不起來。

只有一個來源而且沒寫別名時不加：欄位名稱不可能模稜兩可，補上去只是雜訊。
寫了別名的那一種要加，那不加也執行得動——加的理由是使用者已經用別名說話了，
而他接著多半要再 `JOIN` 一張表進來，那時得靠他自己替展開出來的每一個欄位補前綴。

使用者自己寫了限定字（`a.*`、`dbo.PUBLISHER.*`）時照**原文**帶回去：把
`dbo.PUBLISHER` 改寫成 `PUBLISHER` 雖然也合法，卻是他沒有要求的改動。
