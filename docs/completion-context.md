...
|---|---|---|---|
| （無） | — | 本地物件與關鍵字；`FROM`／`JOIN` 之後另加資料庫與連結伺服器 | 看 `QualifyObjectNames` |
...
資料庫與連結伺服器提交時**帶著點號**寫進去，清單接著開下一段，所以
`SQL 伺服器 → 資料庫 → 物件` 可以一路 Tab 下去。「要不要接著開」問的是真的寫進去的
文字有沒有以點號結尾，而不是建議項上的旗標：同一筆資料庫建議在 `USE` 之後是整句的
終點、在 `FROM` 之後才是中間段，而旗標在建立清單時就決定了，那時還不知道是哪一種。
