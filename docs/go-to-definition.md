...
| 同義字、序列、資料表型別 | 整段註解，寫明為什麼組不出可執行的指令碼 |

前三列與浮動預覽的**指令碼**分頁是同一份（[structure-preview.md](structure-preview.md)），
...
資料表**不**改寫。`ALTER TABLE` 沒有對應的整體寫法，改下去得到的是一段執行到一半
才失敗的指令碼。

### 為什麼有些種類整段是註解

同義字指向哪個物件、序列的起始值與增量、資料表型別的 `CREATE TYPE ... AS TABLE`
語法，這三樣都不在 SqlAssist 查詢的中繼資料裡。硬湊一份出來就是指令碼在說謊：
資料表型別有欄位，不擋掉的話會被寫成 `CREATE TABLE`，照著執行會多出一張同名的
資料表。這與檢視取不到定義時不能掉進 `CREATE TABLE` 是同一條理由。
