...
述詞的起點（`ON |`、`WHERE |`、`AND |`）另外要算得上「判得出來」才參與：那裡收斂的是
`CompletionTarget.Predicate`，與 `Any` 允許的類別一模一樣，差別只在空前綴時參不參與。
少了這一格，打完 `ON ` 的當下目標仍是 `Any`，整份上下文不參與，清單根本不出現——
而那正是使用者要挑聯結欄位的時候。那一格還把當前來源的同名欄位配成整條聯結條件排到
最前面，見[配對鍵](completion-join-keys.md)。
