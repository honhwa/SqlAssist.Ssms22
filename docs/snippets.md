...
`be`（BEGIN END）與 `tc`（TRY CATCH）做的事和[自動配對的區塊骨架](auto-pairing.md)
重疊，但兩者**不衝突**，因為觸發條件不同：

| | 觸發 | 內容 |
|---|---|---|
| 片段 `be`、`tc` | 打捷徑（`be` + Tab） | 帶 Tab Stop 欄位，可以一格一格填 |
| 自動配對的區塊骨架 | 先**選取一段既有文字**再打 `BEGIN` | 把選好的那一段包起來，沒有欄位 |

片段是「從無到有建一個區塊」，骨架是「把已經寫好的東西包起來」，兩邊都各自
需要。`tc` 還會帶 `THROW;`，而骨架刻意不帶——那是錯誤處理的策略，不是語法。
...
自動大寫接著，而別名那一格本來就不開清單（見[沒有 AS 的別名](completion-boundaries.md#沒有-as-的別名靠換行分辨)）。
