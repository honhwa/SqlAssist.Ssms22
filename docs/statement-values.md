...
**每一個**參數都宣告，不只是 `OUTPUT` 那幾個：展開出來的骨架本來就是要使用者接著改值的
半成品，而改值最自然的位置就是變數本身——整份宣告在上、呼叫在下，一次改一處。

`OUTPUT` 在宣告裡的位置是**型別之後、等號之前**：
...
T-SQL 的宣告語法是 `DECLARE @變數 型別 [= 值]`，而 `OUTPUT` 屬於參數修飾詞，只能緊跟在
型別之後。寫在值後面那一版編譯不過，而錯的那一行是使用者按下去之後才看到的。
少了 `OUTPUT` 則程序寫不回值，錯誤訊息（「參數不是 OUTPUT 參數」）出現在呼叫那一行，
跟宣告隔了好幾行。`OUTPUT` 這個字因此出現在兩處：宣告行的型別之後，以及呼叫那一側的傳值位置。

使用者已經宣告過同名變數時會撞名，但那是一個當場看得見的編譯錯誤；
少了 `DECLARE` 則是連編譯都過不了，那比什麼都不做糟糕。

讀預設值的那一段必須把 `OUTPUT`／`OUT`／`READONLY` 這三個修飾詞當成邊界，
**而且這三個字可以寫在等號前面**：

```text
@cond VARCHAR(8000) OUTPUT = ''      -- 修飾詞在等號前
@Result INT = 0 OUTPUT               -- 修飾詞在等號後
```

不認它們的話，前者被讀成「沒有預設值」（修飾詞擋在等號前面），後者切出 `'' OUTPUT`
這種值。兩者都會讓展開後的宣告行多一個 `OUTPUT`——錯誤出現在使用者按下去之後才看得到的
那一行，看起來像展開壞了。
