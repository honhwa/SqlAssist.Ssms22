...
清單每重排一次就把選取放回排名第一的那一筆（正式選取，Enter 提交的就是它），
所以打了前綴直接按 `Enter` 帶出來的是最前面那一個；`↑`、`↓` 挑過之後以挑的那一筆為準。
`FROM `、`ON `、`WHERE ` 之後的物件清單同理：命中項目已排好名次，不打方向鍵直接按 `Enter`
帶出的就是第一個匹配到的物件。
...
函式篩選分成「內建函式」（例如 `AVG`）、「純量函式」與「資料表值函式」。
內嵌與多敘述資料表值函式的呼叫位置相同，合用一顆按鈕；詳細種類仍顯示在項目說明與
結構預覽，不為實作方式增加一排難辨識的按鈕。

CTE、暫存資料表與資料表變數歸在「資料表」那一顆：那顆按鈕問的是「這個位置我要一張
表」，而它們在那個位置就是表；獨立一顆會讓按下「資料表」之後，使用者上一行才寫下的
`#Loan` 反而消失。

篩選列只在候選包含不同分類時出現，且只列當次候選具有的分類。未選取時顯示全部，
多選取聯集；不改變 SQL 語境、排名或插入文字。篩選器實體與順序固定，保留平台的選取狀態。
