...
輸入時彈出的建議清單：排名、觸發時機、上下文收斂，以及清單內容的四個來源
（T-SQL 關鍵字、內建函式、程式碼片段、資料庫物件）。程式碼片段的格式與管理見
...
與 Schema。全域變數（`@@ROWCOUNT`…）只在打出 `@@` 之後單獨出現，見下。
...
型別（`CAST(x AS`）、執行身分（`EXECUTE AS`）都在其餘那一邊。比的是整個位置值相等
...
列舉裡沒有它們。已淘汰的 `@@REMSERVER` 刻意不收——建議一個被移除的名稱比少一個糟。

### 只在 `@@` 之後出現

全域變數不混進一般清單。它們是**唯一**由「正在輸入的詞元」而不是由前導關鍵字決定的
目標：`@@` 開頭的名稱在 T-SQL 裡只有這一種意思，前面是哪一個子句都不改變這件事。
反過來把 31 個 `@@` 塞進每一次按鍵的候選清單，只會讓真正要找的東西更難找。
...
`DECLARE @`、`SELECT @`、`WHERE a = @`、`CREATE PROCEDURE p @` 之後照舊什麼都不列。
那是使用者自己取的名字，清單裡沒有一項會是對的，而彈出來的唯一效果是他順手按下
Enter，剛打的 `@pub` 被換掉——要按復原才救得回來。

分辨的依據就是前綴本身，不是周圍的文法：兩個小老鼠開頭是系統的封閉清單，
一個是使用者的名字。
