...
`FROM (SELECT …) ` 之後文法上**只能**是別名——少了它 `FROM (SELECT 1)` 就是語法
錯誤。那裡清單裡沒有一項會是對的，而彈出來的唯一效果是使用者順手按下 Enter，
剛打的 `a` 被換成 `ALTER PROCEDURE`，得按復原才救得回來。因此那個位置整份不參與，
少的只是 `AS` 兩個字母的補字。

括號是什麼由它**前面**那個字決定，而不是由裡面裝什麼決定：同樣裝著一個 `SELECT`，
接在 `FROM`、`JOIN`、`APPLY` 後面的是衍生資料表，接在 `IN`、`EXISTS`、`=` 後面的
是運算式，後面不接別名。`FROM (t1 JOIN t2 ON …) ` 也不算——那是括號包起來的聯結，
別名反而不合法。
