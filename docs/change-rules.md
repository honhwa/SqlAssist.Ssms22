...
- **禁止**讓連結伺服器（四段式名稱）走一般的中繼資料載入。延遲與可用性都由對方
  那台伺服器決定，而目錄的載入閘是每個目錄一把，卡住的那一輪會擋住同一個資料庫的
  後續查詢；登入失敗還會冒出 SSMS 自己的對話框，那不是 `DbException` 的降級接得住的。
  一律回傳「沒有建議」，理由見 [docs/metadata.md](metadata.md)。
